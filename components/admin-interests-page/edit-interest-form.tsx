...
const EditInterestForm: FC<Props> = ({ interest }): JSX.Element => {
  const router = useRouter();
  const [thumbnail, setThumbnail] = useState(interest?.thumbnail.url || "");
  ...
  const onSubmit = async (formData: FormValues) => {
    try {
      ...
      if (data.ok) {
        setIsLoading(false);
        toast.success(`Cập nhật sở thích ${formData.name} thành công`);
        return router.back();
      }
    } catch (error: any) {
      setIsLoading(false);
