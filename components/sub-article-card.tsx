...
const SubArticleCard: FC<Props> = ({ image, title, slug }): JSX.Element => {
  return (
    <Link
      href={slug}
      className="block w-full aspect-[1.5] rounded-md relative overflow-hidden"
    >
      <NextImage
        src={image}
        alt={title}
      ...
      <div className="bg-transparent absolute top-6 right-10 left-10">
        <span className="text-lg bg-white rounded-md leading-10 py-1 box-decoration-clone shadow-[0.4em_0_0_#fff,-0.4em_0_0_#fff]">
          {title}
        </span>
      </div>
    </Link>
