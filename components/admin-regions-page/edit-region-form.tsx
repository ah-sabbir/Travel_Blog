...
const EditRegionForm: FC<Props> = ({ region }): JSX.Element => {
  const router = useRouter();
  const [thumbnail, setThumbnail] = useState(region?.thumbnail.url || "");
  const [banner, setBanner] = useState(region?.banner.url || "");
  ...
  const onSubmit = async (formData: FormValues) => {
    try {
      ...
      if (data.ok) {
        setIsLoading(false);
        toast.success(`Cập nhật tỉnh / vùng miền ${formData.name} thành công`);
        return router.back();
      }
    } catch (error: any) {
      setIsLoading(false);
  ...
  useEffect(() => {
    const selectedCountryName = countries?.find(
      (item) => item.value === region?.countryId
    )?.label;
          ...
          >
            {thumbnail ? (
              ...
              <>
                ...
                <span className="mt-1 text-slate-700">( Tỷ lệ ảnh: 1.78 )</span>
              </>
            )}
