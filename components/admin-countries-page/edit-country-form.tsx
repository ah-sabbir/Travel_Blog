...
const EditCountryForm: FC<Props> = ({ country }): JSX.Element => {
  const router = useRouter();
  const [thumbnail, setThumbnail] = useState(country?.thumbnail.url || "");
  ...
  const onSubmit = async (formData: FormValues) => {
    try {
      ...
      if (data.ok) {
        setIsLoading(false);
        toast.success(`Cập nhật quốc gia ${formData.name} thành công`);
        return router.back();
      }
    } catch (error: any) {
      setIsLoading(false);
