...
const CreateBrandForm: FC<Props> = (props): JSX.Element => {
  ...
  const onSubmit = async (formData: FormValues) => {
    try {
      ...
      const requestBody: CreateBrandInput = {
        ...formData,
        content,
        logo,
      ...
      if (data.ok) {
        setIsLoading(false);
        toast.success(`Tạo thương hiệu ${formData.name} thành công`);
        return router.back();
      }
    } catch (error: any) {
      setIsLoading(false);
